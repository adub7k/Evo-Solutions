...
| "10-year warranty against yellowing and cracking" on PPF                      | Same. Nothing is published for PPF — the `/ppf` ad landing page has a warranty panel wired but dark (`site.ppfSpecs.warranty` is null)                                                                                                                   | The PPF manufacturer's warranty document, worded exactly as it should appear                                                                                                                                                                                                                                                                            |
| Coating warranty (previously unstated)                                        | **RESOLVED 2026-09-04.** You confirmed the coating is **Nasiol** with an **optional, paid 10-year paint warranty covering stains and etching**. Published on `/ceramic-coating` (hero, guarantee panel, FAQ) exactly in those words                      | **Coating is now QUOTE-ONLY by your decision (2026-09-04)** — no table, no "from $" card; the page says call or send photos. (Your $950 new-sedan / $1,100 base figures stay in ShopFlow for the CRM and are not shown.) Still wanted: the written warranty terms (what voids it, does it transfer) and which Nasiol product line, if you want it named |
...
## 2. Pricing — SWITCHED OFF 2026-09-16 (your call: no prices on the website)

`site.publishPrices` is `false`, so nothing below is currently shown anywhere —
no tables, no "from $" cards, no landing-page price lines, no deposit note.
The wiring is intact: flip that one flag and the live ShopFlow prices return.
The section below describes what shows when it's on.

### When switched on — live from ShopFlow
...
- **`/ppf` — the paid-search landing page for PPF.** Point Google Ads at
  `https://www.evosolution.org/ppf?src=google` (ad-group anchors and the RSA
  copy are in `docs/ppf-rsa-landing.md`). `noindex`, same lead endpoint. With
  prices off, the page promises a flat number by text and shows a click-to-call
  button beside every quote form.
